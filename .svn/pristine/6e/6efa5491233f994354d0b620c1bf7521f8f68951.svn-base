...
import mx.events.DynamicEvent;
	...
	public function processingReturned(event:ResultEvent):void {
		...
		if (event.target.service == "Z_CAS_LEAVE_APPROVEService") {
			XMLMessList = wsDoApprove.Z_CAS_LEAVE_APPROVE.lastResult.RE_T_MESSAGES.item;
		}
		else if (event.target.service == "Z_CAS_LEAVE_REJECTService") {
			XMLMessList = wsDoReject.Z_CAS_LEAVE_REJECT.lastResult.RE_T_MESSAGES.item;
		}
		else if (event.target.service == "Z_CAS_LEAVE_HOLDService") {
			XMLMessList = wsDoHold.Z_CAS_LEAVE_HOLD.lastResult.RE_T_MESSAGES.item;
		}
		else if (event.target.service == "Z_CAS_LEAVE_UNHOLDService") {
			XMLMessList = wsDoUnHold.Z_CAS_LEAVE_UNHOLD.lastResult.RE_T_MESSAGES.item;
		}
		...
		if (event.target.service == "Z_CAS_LEAVE_REJECTService" || event.target.service == "Z_CAS_LEAVE_APPROVEService"){
			doRefresh();			
		}
	}
            
	public function doApprove():void {
       	wsDoApprove.Z_CAS_LEAVE_APPROVE.request.ITEMS = getSelected();
		var notHold:Boolean = true;
		var lineCount:int = 0;
       	for each (var x:XML in wsDoApprove.Z_CAS_LEAVE_APPROVE.request.ITEMS)
       	{
		...
		if (lineCount == 0) {
			Alert.show("Please select at least 1 item to approve");
           	...
           	wsDoApprove.Z_CAS_LEAVE_APPROVE.send();  	
	    }
		else
	    {
	...
	public function doReject():void {
    	wsDoReject.Z_CAS_LEAVE_REJECT.request.ITEMS = getSelected();
		var reason:Boolean = true;
		var notHold:Boolean = true;
		var lineCount:int = 0;
       	for each (var x:XML in wsDoReject.Z_CAS_LEAVE_REJECT.request.ITEMS)
       	{
       		lineCount = lineCount + 1;
		...
		if (lineCount == 0) {
			Alert.show("Please select at least 1 item to reject");
       		...
       		wsDoReject.Z_CAS_LEAVE_REJECT.send();  	
	    }
	    else if (reason != true)
	    {
    ...
    // Get selected items
       	...
       	var allItems:XMLList = new XMLList(wsGetItemsToApprove.Z_CAS_LEAVE_GET_ITEMS.lastResult..item);
	...
	public function setOnHold(WIID:String, holduntil:String, holdtext:String):void
	{
		...
		holdDate = holduntil;
		holdReason = holdtext;
		wsDoHold.Z_CAS_LEAVE_HOLD.send();
	}
	...
	public function setOnUnHold(WIID:String, holduntil:String, holdtext:String):void
	{
		...
		holdDate = holduntil;
		holdReason = holdtext;			
		wsDoUnHold.Z_CAS_LEAVE_UNHOLD.send();
	}	
		
	// return the history for the line selected
	public function getHistory(parent_wiid:String):XMLList
	{
		var outerLines:* = wsGetItemsToApprove.Z_CAS_LEAVE_GET_ITEMS.lastResult.HR_HISTORY.item;
		var lines:XMLList = wsGetItemsToApprove.Z_CAS_LEAVE_GET_ITEMS.lastResult.HR_HISTORY.item.(PARENT_WIID == parent_wiid);			
		if (lines.length() > 0)
			return lines;
		else
			return null;
	...
	{
		var outerLines:* = wsGetItemsToApprove.Z_CAS_LEAVE_GET_ITEMS.lastResult.LEAVE_NOTES.item;
		var lines:XMLList = wsGetItemsToApprove.Z_CAS_LEAVE_GET_ITEMS.lastResult.LEAVE_NOTES.item.(PARENT_WIID == parent_wiid);			
		if (lines.length() > 0)
			return lines;
		else
			return null;
