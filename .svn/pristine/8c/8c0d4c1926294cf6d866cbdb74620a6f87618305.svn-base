...
import mx.events.DynamicEvent;
	...
	{
		var outerLines:* = wsGetItemsToReport.Z_CAS_LEAVE_REPORTING.lastResult.HR_HISTORY.item;
		var lines:XMLList = wsGetItemsToReport.Z_CAS_LEAVE_REPORTING.lastResult.HR_HISTORY.item.(PARENT_WIID == parent_wiid);			
		if (lines.length() > 0)
			return lines;
		else
			return null;
	...
	{
		var outerLines:* = wsGetItemsToReport.Z_CAS_LEAVE_REPORTING.lastResult.LEAVE_NOTES.item;
		var lines:XMLList = wsGetItemsToReport.Z_CAS_LEAVE_REPORTING.lastResult.LEAVE_NOTES.item.(PARENT_WIID == parent_wiid);			
		if (lines.length() > 0)
			return lines;
		else
			return null;
