...
import mx.events.DynamicEvent;
	...
	public function processingReturned(event:ResultEvent):void {
		...
		if (event.target.service == "Z_CAS_HR_APPROVEService") {
			XMLMessList = wsDoApprove.Z_CAS_HR_APPROVE.lastResult.RE_T_MESSAGES.item;
		}
		else if (event.target.service == "Z_CAS_HR_REJECTService") {
			XMLMessList = wsDoReject.Z_CAS_HR_REJECT.lastResult.RE_T_MESSAGES.item;
		}
		else if (event.target.service == "Z_CAS_HR_HOLDService") {
			XMLMessList = wsDoHold.Z_CAS_HR_HOLD.lastResult.RE_T_MESSAGES.item;
		}
		else if (event.target.service == "Z_CAS_HR_UNHOLDService") {
			XMLMessList = wsDoUnHold.Z_CAS_HR_UNHOLD.lastResult.RE_T_MESSAGES.item;
		}
		...
		if (event.target.service != "Z_CAS_HR_HOLDService" && event.target.service != "Z_CAS_HR_UNHOLDService"){
			doRefresh();			
		}
	}
	...
	public function doApprove():void {
		...
		var lineCount:int = 0;
      	for each (var x:XML in wsDoApprove.Z_CAS_HR_APPROVE.request.ITEMS)
       	{
	...
	public function doReject():void {
		...
		var notHold:Boolean = true;
		var lineCount:int = 0;
       	for each (var x:XML in wsDoReject.Z_CAS_HR_REJECT.request.ITEMS)
       	{
       		lineCount = lineCount + 1;
...
//		var l_children:* = repeaterContainer.getChildren();				
	...
	public function setOnUnHold(WIID:String, holduntil:String, holdtext:String):void
	{
		...
		holdDate = holduntil;
		holdReason = holdtext;			
		wsDoUnHold.Z_CAS_HR_UNHOLD.send();
	}	
		
	// return the history for the line selected
	public function getHistory(parent_wiid:String):XMLList
	{
		var outerLines:* = wsGetItemsToApprove.Z_CAS_HR_GET_ITEMS.lastResult.HR_HISTORY.item;
		var lines:XMLList = wsGetItemsToApprove.Z_CAS_HR_GET_ITEMS.lastResult.HR_HISTORY.item.(PARENT_WIID == parent_wiid);			
		if (lines.length() > 0)
			return lines;
		else
			return null;
