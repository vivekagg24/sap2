...
import mx.events.DynamicEvent;
	...
	public function processingReturned(event:ResultEvent):void {
		...
		if (event.target.service == "Z_CAS_MISC_APPROVEService") {
			XMLMessList = wsDoApprove.Z_CAS_MISC_APPROVE.lastResult.RE_T_MESSAGES.item;
		}
		else if (event.target.service == "Z_CAS_MISC_REJECTService") {
			XMLMessList = wsDoReject.Z_CAS_MISC_REJECT.lastResult.RE_T_MESSAGES.item;
		}
		else if (event.target.service == "Z_CAS_MISC_HOLDService") {
			XMLMessList = wsDoHold.Z_CAS_MISC_HOLD.lastResult.RE_T_MESSAGES.item;
		}
		else if (event.target.service == "Z_CAS_MISC_UNHOLDService") {
			XMLMessList = wsDoUnHold.Z_CAS_MISC_UNHOLD.lastResult.RE_T_MESSAGES.item;
		}
		...
		if (event.target.service != "Z_CAS_MISC_HOLDService" && event.target.service != "Z_CAS_MISC_UNHOLDService"){
			doRefresh();			
		}
	}
	...
	public function doApprove():void {
		...
		var lineCount:int = 0;
      	for each (var x:XML in wsDoApprove.Z_CAS_MISC_APPROVE.request.ITEMS)
       	{
	...
	public function doReject():void {
		...
		var notHold:Boolean = true;
		var lineCount:int = 0;
       	for each (var x:XML in wsDoReject.Z_CAS_MISC_REJECT.request.ITEMS)
       	{
       		lineCount = lineCount + 1;
...
//		var l_children:* = repeaterContainer.getChildren();				
	...
	public function setOnUnHold(WIID:String, holduntil:String, holdtext:String):void
	{
		...
		holdDate = holduntil;
		holdReason = holdtext;			
		wsDoUnHold.Z_CAS_MISC_UNHOLD.send();
	}	
		
	// return the history for the line selected
	public function getHistory(parent_wiid:String):XMLList
	{
		var outerLines:* = wsGetItemsToApprove.Z_CAS_MISC_GET_ITEMS.lastResult.MISC_HISTORY.item;
		var lines:XMLList = wsGetItemsToApprove.Z_CAS_MISC_GET_ITEMS.lastResult.MISC_HISTORY.item.(PARENT_WIID == parent_wiid);			
		if (lines.length() > 0)
			return lines;
		else
			return null;
