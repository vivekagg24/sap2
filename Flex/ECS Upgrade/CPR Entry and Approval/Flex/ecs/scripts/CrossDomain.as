 ...
 */
    ...
    public function loadCrossPolicyFile(event:Object = ""):void
	{
		...
		var services:Object;
		var i1:int;
		if (loadEvent) // Get the url from the loadEvent
		{
		...
		if (faultEvent) // Get the url from the fault event
		{
			var s:String = event.fault.rootCause.text;
			var i:int = s.indexOf("cannot load data from ");
			if (i == -1)
				return;
			i += 22;
			url = s.slice(i);
